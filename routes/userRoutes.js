...
router.post('/register', async (req, res, next) => {
  try {
    ...
    res.status(201).json({
      msg: 'Registrierung erfolgreich. Bitte überprüfe deine E-Mails.',
    });
  } catch (err) {
...
router.post('/login', async (req, res, next) => {
  ...
  try {
    ...
    if (!user.isConfirmed) {
      return res
        .status(403)
        .json({ msg: 'Bitte bestätige zuerst deine E-Mail Adresse.' });
    }
...
router.post('/tutorial-seen', auth, async (req, res, next) => {
  try {
    ...
    res.status(200).json({ msg: 'Tutorial als gesehen markiert' });
  } catch (err) {
...
router.put('/confirm-email', async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(400)
      .json({ msg: 'Ungültiger oder abgelaufener Bestätigungslink' });
  }
});
